...
<div class="row">
<?php if(count($inspections_years) > 1 || isset($currencies)){ ?>
  <div class="col-md-12 simple-bootstrap-select mbot5">
   <?php if(isset($currencies)){ ?>
    <select class="selectpicker" data-width="auto" name="total_currency" onchange="init_inspection_total();">
    ...
    <?php } ?>
      <?php
      if(count($inspections_years) > 1){ ?>
      <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker" multiple name="inspections_total_years" onchange="init_inspection_total();">
  ...
  foreach($totals as $key => $data){
    $class = inspection_status_color_class($data['status']);
    $name = inspection_status_by_id($data['status']);
    ?>
